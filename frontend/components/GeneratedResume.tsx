  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 24, padding: '36px 40px', boxShadow: '0 4px 28px rgba(15,23,42,0.05)' }} className="animate-fade-slide-up">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <h2 style={{ fontFamily: "'Fira Code', monospace", fontWeight: 800, fontSize: 24, color: C.ink, margin: 0, letterSpacing: '-0.5px' }}>Generated Resume</h2>
        <div style={{ display: 'flex', gap: 12 }}>
          <button
            ...
            style={{
              ...
              color: C.muted, fontFamily: "'Fira Code', monospace", fontWeight: 700, fontSize: 13.5,
      ...
      <div style={{ maxHeight: 500, overflowY: 'auto', paddingRight: 10 }}>
        {/* Name */}
        <h3 style={{ fontFamily: "'Fira Code', monospace", fontWeight: 900, fontSize: 32, color: C.ink, margin: '0 0 24px', letterSpacing: '-1px' }}>{resumeData.name}</h3>

        {/* Summary */}
        {resumeData.professional_summary && (
          <div style={{ marginBottom: 28 }}>
            <h4 style={{ fontFamily: "'Fira Code', monospace", fontSize: 13, fontWeight: 800, color: C.accent, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10 }}>Summary</h4>
            <p style={{ fontFamily: "'Fira Code', monospace", fontSize: 14.5, color: C.muted, lineHeight: 1.6, margin: 0 }}>{resumeData.professional_summary}</p>
          </div>
        )}
        ...
        {resumeData.skills && resumeData.skills.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            <h4 style={{ fontFamily: "'Fira Code', monospace", fontSize: 13, fontWeight: 800, color: C.accent, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 12 }}>Skills</h4>
        ...
        {resumeData.experience && resumeData.experience.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            <h4 style={{ fontFamily: "'Fira Code', monospace", fontSize: 13, fontWeight: 800, color: C.accent, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 16 }}>Experience</h4>
              ...
              {resumeData.experience.map((exp: any, idx: number) => (
                <div key={idx} style={{ background: C.paper, border: `1px solid ${C.border}`, borderRadius: 16, padding: '20px 24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <p style={{ fontFamily: "'Fira Code', monospace", fontWeight: 800, fontSize: 16, color: C.ink, margin: 0 }}>{exp.role}</p>
                    <span style={{ fontFamily: "'Fira Code', monospace", fontSize: 12.5, color: C.muted, fontWeight: 600, background: C.surface, padding: '4px 10px', borderRadius: 6, border: `1px solid ${C.border}` }}>{exp.period || exp.duration || ""}</span>
                  </div>
                  <p style={{ fontFamily: "'Fira Code', monospace", fontSize: 14, color: C.accent, fontWeight: 600, margin: '0 0 12px' }}>{exp.company}</p>
                  <ul style={{ margin: 0, paddingLeft: 18, color: C.muted, fontFamily: "'Fira Code', monospace", fontSize: 14, lineHeight: 1.6 }}>
        ...
        {resumeData.projects && resumeData.projects.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            <h4 style={{ fontFamily: "'Fira Code', monospace", fontSize: 13, fontWeight: 800, color: C.accent, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 16 }}>Projects</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {resumeData.projects.map((p: any, idx: number) => (
                <div key={idx} style={{ background: C.paper, border: `1px solid ${C.border}`, borderRadius: 16, padding: '18px 22px' }}>
                  <p style={{ fontFamily: "'Fira Code', monospace", fontWeight: 800, fontSize: 15, color: C.ink, margin: '0 0 6px' }}>{p.name || p.title}</p>
                  <p style={{ fontFamily: "'Fira Code', monospace", fontSize: 14, color: C.muted, margin: '0 0 10px', lineHeight: 1.5 }}>{p.desc || p.description}</p>
                  {p.bullets && (
                    <ul style={{ margin: 0, paddingLeft: 18, color: C.muted, fontFamily: "'Fira Code', monospace", fontSize: 13.5, lineHeight: 1.6 }}>
